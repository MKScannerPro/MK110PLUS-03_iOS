...
@interface MKRMBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMBleAdvBeaconModel *dataModel;

@end

@implementation MKRMBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKRMBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMBleAdvBeaconController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
