//
//  MKRMBleNetworkSettingsController.m
...
@interface MKRMBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMBleNetworkSettingsModel *dataModel;

@end

@implementation MKRMBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKRMBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMBleNetworkSettingsController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
