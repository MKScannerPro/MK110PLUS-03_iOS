//
//  MKRMBleWifiSettingsController.m
...
#include "MKRMBleWifiSettingsCertCell.h"
#include "MKRMBleWifiSettingsBandCell.h"
...
@interface MKRMBleWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKRMBleWifiSettingsCertCellDelegate,
MKCAFileSelectControllerDelegate,
MKRMBleWifiSettingsBandCellDelegate>
...
@property (nonatomic, strong)MKRMBleWifiSettingsModel *dataModel;

@end

@implementation MKRMBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKRMBleWifiSettingsController销毁");
}
...
#pragma mark - MKRMBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKRMBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKRMBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKRMBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
#pragma mark - MKRMBleWifiSettingsBandCellDelegate
- (void)rm_bleWifiSettingsBandCell_countryChanged:(NSInteger)country {
    MKRMBleWifiSettingsBandCellModel *cellModel = self.section11List[0];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKRMDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKRMDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKRMBleWifiSettingsCertCell *cell = [MKRMBleWifiSettingsCertCell initCellWithTableView:self.tableView];
    ...
    //Country&Band
    MKRMBleWifiSettingsBandCell *cell = [MKRMBleWifiSettingsBandCell initCellWithTableView:self.tableView];
...
- (void)loadSection8Datas {
    MKRMBleWifiSettingsCertCellModel *cellModel = [[MKRMBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection11Datas {
    MKRMBleWifiSettingsBandCellModel *cellModel = [[MKRMBleWifiSettingsBandCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMBleWifiSettingsController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
