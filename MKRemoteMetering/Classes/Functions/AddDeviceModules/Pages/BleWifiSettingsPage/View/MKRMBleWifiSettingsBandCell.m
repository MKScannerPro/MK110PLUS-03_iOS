...
@implementation MKRMBleWifiSettingsBandCellModel
@end

@interface MKRMBleWifiSettingsBandCell ()
...
@implementation MKRMBleWifiSettingsBandCell

+ (MKRMBleWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKRMBleWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRMBleWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKRMBleWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRMBleWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rm_bleWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate rm_bleWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRMBleWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMBleWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
