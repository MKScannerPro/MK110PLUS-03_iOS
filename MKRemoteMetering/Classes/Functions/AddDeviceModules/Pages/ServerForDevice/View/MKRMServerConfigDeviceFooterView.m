//
//  MKRMServerConfigDeviceFooterView.m
...
@implementation MKRMServerConfigDeviceFooterViewModel
@end
...
@interface MKRMServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKRMMQTTSSLForDeviceViewDelegate,
MKRMServerConfigDeviceSettingViewDelegate,
MKRMMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKRMMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKRMMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKRMServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKRMServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rm_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate rm_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rm_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rm_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rm_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKRMMQTTSSLForDeviceViewDelegate
- (void)rm_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rm_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA certificate     1:Self signed certificates
- (void)rm_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate rm_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)rm_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate rm_mqtt_serverForDevice_fileButtonPressed:0];
    }
}

/// 用户点击选择了Client Key按钮
- (void)rm_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate rm_mqtt_serverForDevice_fileButtonPressed:1];
    }
}

/// 用户点击了Client Cert File按钮
- (void)rm_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate rm_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKRMServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)rm_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate rm_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKRMMQTTLWTForDeviceViewDelegate
- (void)rm_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rm_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)rm_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rm_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)rm_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate rm_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)rm_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rm_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)rm_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rm_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKRMServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKRMMQTTSSLForDeviceViewModel *sslModel = [[MKRMMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKRMMQTTLWTForDeviceViewModel *lwtModel = [[MKRMMQTTLWTForDeviceViewModel alloc] init];
...
- (MKRMMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKRMMQTTSSLForDeviceView alloc] init];
...
- (MKRMMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKRMMQTTLWTForDeviceView alloc] init];
...
- (MKRMServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKRMServerConfigDeviceSettingView alloc] init];
