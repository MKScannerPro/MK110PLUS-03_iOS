...
@implementation MKRMMQTTLWTForDeviceViewModel
@end

@interface MKRMMQTTLWTForDeviceView ()
...
@implementation MKRMMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKRemoteMetering", @"MKRMMQTTLWTForDeviceView", @"rm_switchSelectedIcon.png") : LOADICON(@"MKRemoteMetering", @"MKRMMQTTLWTForDeviceView", @"rm_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(rm_lwt_statusChanged:)]) {
        [self.delegate rm_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKRemoteMetering", @"MKRMMQTTLWTForDeviceView", @"rm_switchSelectedIcon.png") : LOADICON(@"MKRemoteMetering", @"MKRMMQTTLWTForDeviceView", @"rm_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(rm_lwt_retainChanged:)]) {
        [self.delegate rm_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(rm_lwt_qosChanged:)]) {
        [self.delegate rm_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRMMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMMQTTLWTForDeviceView", @"rm_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMMQTTLWTForDeviceView", @"rm_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rm_lwt_topicChanged:)]) {
                [self.delegate rm_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rm_lwt_payloadChanged:)]) {
                [self.delegate rm_lwt_payloadChanged:text];
            }
        };
