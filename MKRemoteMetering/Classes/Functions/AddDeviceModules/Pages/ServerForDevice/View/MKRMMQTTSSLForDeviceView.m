...
@implementation MKRMMQTTSSLForDeviceViewModel
@end

@interface MKRMMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKRMMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate rm_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate rm_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate rm_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate rm_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate rm_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRMMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKRemoteMetering", @"MKRMMQTTSSLForDeviceView", @"rm_switchSelectedIcon.png") : LOADICON(@"MKRemoteMetering", @"MKRMMQTTSSLForDeviceView", @"rm_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMMQTTSSLForDeviceView", @"rm_switchUnselectedIcon.png") forState:UIControlStateNormal];
