//
//  MKRMServerConfigDeviceSettingView.m
...
@interface MKRMServerConfigDeviceSettingView ()
...
@implementation MKRMServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate rm_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate rm_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate rm_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
