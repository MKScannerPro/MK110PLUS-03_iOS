//
//  MKRMServerForDeviceController.m
...
@interface MKRMServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKRMServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKRMImportServerControllerDelegate>
...
@property (nonatomic, strong)MKRMServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKRMServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKRMServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKRMServerForDeviceController

- (void)dealloc {
    NSLog(@"MKRMServerForDeviceController销毁");
}
...
#pragma mark - MKRMServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)rm_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)rm_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)rm_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)rm_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)rm_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)rm_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKRMImportServerController *vc = [[MKRMImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKRMImportServerControllerDelegate
- (void)rm_selectedServerParams:(NSString *)fileName {
    [MKRMExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKRMServerForDeviceModel *model = [MKRMServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKRMDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKRMDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKRMDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKRMDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKRMDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKRMDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKRMDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKRMDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKRMExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKRMServerForDeviceModel *emptyModel = [[MKRMServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMServerForDeviceController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKRMServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKRMServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKRMServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKRMServerConfigDeviceFooterViewModel alloc] init];
    }
