//
//  MKRMBleNTPTimezoneController.m
...
@interface MKRMBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKRMBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKRMBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMBleNTPTimezoneController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
