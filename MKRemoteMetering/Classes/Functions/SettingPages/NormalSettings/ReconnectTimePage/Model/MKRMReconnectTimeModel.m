...
@interface MKRMReconnectTimeModel ()
...
@implementation MKRMReconnectTimeModel
...
- (BOOL)readReconnectTimeout {
    ...
    [MKRMMQTTInterface rm_readNetworkReconnectTimeoutWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReconnectTimeout {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configReconnectTimeout:[self.timeout integerValue] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
