...
@interface MKRMSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKRMSystemTimeCellDelegate,
...
@implementation MKRMSystemTimeController

- (void)dealloc {
    NSLog(@"MKRMSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKRMNTPServerController *vc = [[MKRMNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKRMSystemTimeCell *cell = [MKRMSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKRMSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKRMMQTTInterface rm_readDeviceUTCTimeWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKRMMQTTInterface rm_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKRMSystemTimeCellModel *cellModel = [[MKRMSystemTimeCellModel alloc] init];
    cellModel.index = 0;
