...
@interface MKRMCommunicateModel ()
...
@implementation MKRMCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKRMMQTTInterface rm_readCommunicateTimeoutWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
