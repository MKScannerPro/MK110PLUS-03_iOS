...
@interface MKRMAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMAdvBeaconModel *dataModel;

@end

@implementation MKRMAdvBeaconController

- (void)dealloc {
    NSLog(@"MKRMAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMAdvBeaconController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
