//
//  MKRMIndicatorSettingsController.m
...
@interface MKRMIndicatorSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMIndicatorSetingsModel *dataModel;

@end

@implementation MKRMIndicatorSettingsController

- (void)dealloc {
    NSLog(@"MKRMIndicatorSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMIndicatorSettingsController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMIndicatorSetingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMIndicatorSetingsModel alloc] init];
    }
    return _dataModel;
}
