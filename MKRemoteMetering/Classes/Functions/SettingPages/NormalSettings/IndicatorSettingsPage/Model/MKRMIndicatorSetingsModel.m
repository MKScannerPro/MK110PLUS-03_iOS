...
@interface MKRMIndicatorSetingsModel ()
...
@implementation MKRMIndicatorSetingsModel
...
- (BOOL)readIndicatorLightStatus {
    ...
    [MKRMMQTTInterface rm_readIndicatorLightStatusWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIndicatorLightStatus {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configIndicatorLightStatus:self macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
