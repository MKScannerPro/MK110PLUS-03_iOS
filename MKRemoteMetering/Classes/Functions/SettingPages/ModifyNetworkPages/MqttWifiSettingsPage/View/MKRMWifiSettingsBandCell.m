...
@implementation MKRMWifiSettingsBandCellModel
@end

@interface MKRMWifiSettingsBandCell ()
...
@implementation MKRMWifiSettingsBandCell

+ (MKRMWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKRMWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRMWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKRMWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRMWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rm_wifiSettingsBandCell_countryChanged:)]) {
            [self.delegate rm_wifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRMWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
