//
//  MKRMMqttWifiSettingsController.m
...
@interface MKRMMqttWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKRMWifiSettingsBandCellDelegate>
...
@property (nonatomic, strong)MKRMMqttWifiSettingsModel *dataModel;

@end

@implementation MKRMMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKRMMqttWifiSettingsController销毁");
}
...
#pragma mark - MKRMWifiSettingsBandCellDelegate
- (void)rm_wifiSettingsBandCell_countryChanged:(NSInteger)country {
    MKRMWifiSettingsBandCellModel *cellModel = self.section11List[0];
            ...
            if (((self.dataModel.eapType == 0 || self.dataModel.eapType == 1) && self.dataModel.verifyServer) || self.dataModel.eapType == 2) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(receiveUpdateEAPCerts:)
                                                             name:MKRMReceiveDeviceUpdateEapCertsResultNotification
                                                           object:nil];
                return;
            }
        }
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //Country&Band
    MKRMWifiSettingsBandCell *cell = [MKRMWifiSettingsBandCell initCellWithTableView:self.tableView];
...
- (void)loadSection11Datas {
    MKRMWifiSettingsBandCellModel *cellModel = [[MKRMWifiSettingsBandCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMMqttWifiSettingsController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
