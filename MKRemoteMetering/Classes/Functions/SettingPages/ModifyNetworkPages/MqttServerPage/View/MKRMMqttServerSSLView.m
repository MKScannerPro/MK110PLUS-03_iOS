...
@implementation MKRMMqttServerSSLViewModel
@end

@interface MKRMMqttServerSSLView ()<MKRMMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKRMMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKRMMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKRMMqttServerSSLTextField *clientCertView;

@end

@implementation MKRMMqttServerSSLView
...
#pragma mark - MKRMMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)rm_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate rm_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate rm_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rm_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate rm_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRMMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKRMMqttServerSSLTextFieldModel *caModel = [[MKRMMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKRMMqttServerSSLTextFieldModel *clientKeyModel = [[MKRMMqttServerSSLTextFieldModel alloc] init];
    ...
    MKRMMqttServerSSLTextFieldModel *clientModel = [[MKRMMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKRemoteMetering", @"MKRMMqttServerSSLView", @"rm_switchSelectedIcon.png") : LOADICON(@"MKRemoteMetering", @"MKRMMqttServerSSLView", @"rm_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMMqttServerSSLView", @"rm_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKRMMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKRMMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKRMMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKRMMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKRMMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKRMMqttServerSSLTextField alloc] init];
