//
//  MKRMMqttServerConfigFooterView.m
...
@implementation MKRMMqttServerConfigFooterViewModel
@end
...
@interface MKRMMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKRMMqttServerSSLViewDelegate,
MKRMMqttServerSettingViewDelegate,
MKRMMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKRMMqttServerSSLView *sslView;

@property (nonatomic, strong)MKRMMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKRMMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKRMMqttServerConfigFooterView
...
#pragma mark - MKRMMqttServerSSLViewDelegate
- (void)rm_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)rm_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)rm_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(rm_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rm_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKRMMqttServerSettingViewDelegate
...
#pragma mark - MKRMMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKRMMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKRMMqttServerSSLViewModel *sslModel = [[MKRMMqttServerSSLViewModel alloc] init];
    ...
    MKRMMqttServerLwtViewModel *lwtModel = [[MKRMMqttServerLwtViewModel alloc] init];
...
- (MKRMMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKRMMqttServerSSLView alloc] init];
...
- (MKRMMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKRMMqttServerLwtView alloc] init];
...
- (MKRMMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKRMMqttServerSettingView alloc] init];
