...
@interface MKRMMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKRMMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKRMMqttServerModel *dataModel;

@property (nonatomic, strong)MKRMMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKRMMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKRMMqttServerController

- (void)dealloc {
    NSLog(@"MKRMMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKRMReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKRMMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)rm_selectedServerParams:(NSString *)fileName {
    [MKRMExcelDataManager parseDeviceExcel:fileName
        ...
        MKRMMqttServerModel *model = [MKRMMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKRMMqttServerModel *model = [[MKRMMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMMqttServerController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKRMMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKRMMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKRMMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKRMMqttServerConfigFooterViewModel alloc] init];
    }
