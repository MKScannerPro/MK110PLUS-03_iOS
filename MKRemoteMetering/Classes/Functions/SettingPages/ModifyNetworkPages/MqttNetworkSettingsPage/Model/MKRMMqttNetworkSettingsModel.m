//
//  MKRMMqttNetworkSettingsModel.m
...
@interface MKRMMqttNetworkSettingsModel ()
...
@implementation MKRMMqttNetworkSettingsModel
...
- (BOOL)readNetworkInfos {
    ...
    [MKRMMQTTInterface rm_readNetworkInfosWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkInfos {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_modifyNetworkInfos:self macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
