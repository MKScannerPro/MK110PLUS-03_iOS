...
@interface MKRMServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKRMServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKRMServerForAppModel *dataModel;

@property (nonatomic, strong)MKRMServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKRMServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKRMServerForAppController

- (void)dealloc {
    NSLog(@"MKRMServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKRMMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKRMMQTTDataManager shared] connect];
}
...
#pragma mark - MKRMServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)rm_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)rm_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)rm_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)rm_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)rm_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)rm_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)rm_selectedServerParams:(NSString *)fileName {
    [MKRMExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKRMServerForAppModel *model = [MKRMServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKRMMQTTDataManager shared].state == MKRMMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKRMMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_rm_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKRMExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKRMMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKRMMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMServerForAppController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKRMServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKRMServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKRMServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKRMServerConfigAppFooterViewModel alloc] init];
    }
