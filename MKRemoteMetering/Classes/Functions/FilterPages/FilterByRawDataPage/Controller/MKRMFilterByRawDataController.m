//
//  MKRMFilterByRawDataController.m
...
@interface MKRMFilterByRawDataController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMFilterByRawDataModel *dataModel;

@end

@implementation MKRMFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKRMFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKRMFilterByBeaconController *vc = [[MKRMFilterByBeaconController alloc] init];
        vc.pageType = mk_rm_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKRMFilterByUIDController *vc = [[MKRMFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKRMFilterByURLController *vc = [[MKRMFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKRMFilterByTLMController *vc = [[MKRMFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKRMFilterByButtonController *vc = [[MKRMFilterByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP-Tag
        MKRMFilterByTagController *vc = [[MKRMFilterByTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //PIR
        MKRMFilterByPirController *vc = [[MKRMFilterByPirController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 3) {
        //Other
        MKRMFilterByOtherController *vc = [[MKRMFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configFilterBXPDeviceInfo:(BOOL)isOn {
    ...
    [MKRMMQTTInterface rm_configFilterFilterBXPDeviceInfo:isOn macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPACC:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKRMMQTTInterface rm_configFilterBXPAcc:isOn macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKRMMQTTInterface rm_configFilterBXPTH:isOn macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (MKRMFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
