...
@interface MKRMUploadDataOptionModel ()
...
@implementation MKRMUploadDataOptionModel
...
- (BOOL)readUploadDataOption {
    ...
    [MKRMMQTTInterface rm_readUploadDataOptionWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configUploadDataOption {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configUploadDataOption:self macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
