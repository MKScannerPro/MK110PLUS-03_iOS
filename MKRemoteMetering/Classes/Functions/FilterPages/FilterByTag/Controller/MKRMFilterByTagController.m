...
@interface MKRMFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMFilterByTagModel *dataModel;

@end

@implementation MKRMFilterByTagController

- (void)dealloc {
    NSLog(@"MKRMFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMFilterByTagController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMFilterByTagModel alloc] init];
    }
    return _dataModel;
}
