...
@implementation MKRMFilterRawAdvDataModel
...
@interface MKRMFilterByOtherModel ()
...
@implementation MKRMFilterByOtherModel
...
- (BOOL)readFilterByOther {
    ...
    [MKRMMQTTInterface rm_readFilterOtherDatasWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByOther:(NSArray <MKRMFilterRawAdvDataModel *>*)list relationship:(mk_rm_filterByOther)relationship {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configFilterByOtherDatas:self.isOn relationship:relationship rawDataList:list macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
