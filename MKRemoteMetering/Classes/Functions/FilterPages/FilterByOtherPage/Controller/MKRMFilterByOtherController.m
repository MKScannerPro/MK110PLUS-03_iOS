...
@interface MKRMFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKRMFilterEditSectionHeaderViewDelegate,
MKRMFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKRMFilterByOtherModel *dataModel;

@end

@implementation MKRMFilterByOtherController

- (void)dealloc {
    NSLog(@"MKRMFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        ...
        MKRMFilterEditSectionHeaderViewModel *model = [[MKRMFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKRMFilterByRawDataCell *cell = [MKRMFilterByRawDataCell initCellWithTableView:tableView];
...
- (void)mk_rm_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKRMFilterByRawDataCellModel *cellModel = [[MKRMFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKRMFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)rm_rawFilterDataChanged:(mk_rm_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKRMFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_rm_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_rm_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_rm_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_rm_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKRMFilterByRawDataCellModel *cellModel = self.section1List[i];
        ...
        MKRMFilterRawAdvDataModel *model = [[MKRMFilterRawAdvDataModel alloc] init];
...
- (mk_rm_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_rm_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_rm_filterByOther_AB;
        }
        return mk_rm_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_rm_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_rm_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_rm_filterByOther_AOrBOrC;
        }
    }
    return mk_rm_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMFilterByOtherController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
