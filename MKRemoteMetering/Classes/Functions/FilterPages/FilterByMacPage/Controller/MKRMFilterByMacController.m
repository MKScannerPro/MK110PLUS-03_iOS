...
@interface MKRMFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMFilterByMacModel *dataModel;

@end

@implementation MKRMFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMFilterByMacController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMFilterByMacModel alloc] init];
    }
    return _dataModel;
}
