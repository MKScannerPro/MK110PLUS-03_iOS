...
@interface MKRMFilterByMacModel ()
...
@implementation MKRMFilterByMacModel
...
- (BOOL)readFilterByMac {
    ...
    [MKRMMQTTInterface rm_readFilterByMacWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByMac:(NSArray <NSString *>*)macList {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configFilterByMacAddress:macList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
