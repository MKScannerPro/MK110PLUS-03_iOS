//
//  MKRMDuplicateDataFilterModel.m
...
@interface MKRMDuplicateDataFilterModel ()
...
@implementation MKRMDuplicateDataFilterModel
...
- (BOOL)readDuplicateDataFilter {
    ...
    [MKRMMQTTInterface rm_readDuplicateDataFilterDatasWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDuplicateDataFilter {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configDuplicateDataFilter:self.rule period:[self.time longLongValue] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
