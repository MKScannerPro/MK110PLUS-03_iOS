//
//  MKRMDuplicateDataFilterController.m
...
@interface MKRMDuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMDuplicateDataFilterModel *dataModel;

@end

@implementation MKRMDuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKRMDuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMDuplicateDataFilterController", @"rm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRMDuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMDuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
