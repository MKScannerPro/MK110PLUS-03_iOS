...
@interface MKRMFilterByUIDModel ()
...
@implementation MKRMFilterByUIDModel
...
- (BOOL)readFilterByUID {
    ...
    [MKRMMQTTInterface rm_readFilterByUIDWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByUID {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configFilterByUID:self.isOn namespaceID:self.namespaceID instanceID:self.instanceID macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
