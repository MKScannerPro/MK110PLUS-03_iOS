...
@interface MKRMFilterByAdvNameModel ()
...
@implementation MKRMFilterByAdvNameModel
...
- (BOOL)readFilterByName {
    ...
    [MKRMMQTTInterface rm_readFilterByADVNameWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByName:(NSArray <NSString *>*)nameList {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configFilterByADVName:nameList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
