...
@implementation MKRMFilterCellModel
@end

@interface MKRMFilterCell ()
...
@implementation MKRMFilterCell

+ (MKRMFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKRMFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRMFilterCellIdenty"];
    if (!cell) {
        cell = [[MKRMFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRMFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rm_filterValueChanged:index:)]) {
            [self.delegate rm_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRMFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMFilterCellModel.class]) {
        return;
    }
