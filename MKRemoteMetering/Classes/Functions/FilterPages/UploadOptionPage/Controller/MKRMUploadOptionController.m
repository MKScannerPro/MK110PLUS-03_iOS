...
@interface MKRMUploadOptionController ()<UITableViewDelegate,
UITableViewDataSource,
MKNormalSliderCellDelegate,
MKRMFilterCellDelegate>
...
@property (nonatomic, strong)MKRMUploadOptionModel *dataModel;

@end

@implementation MKRMUploadOptionController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKRMFilterCell *cell = [MKRMFilterCell initCellWithTableView:tableView];
...
- (void)rm_filterValueChanged:(NSInteger)dataListIndex index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKRMFilterCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)filterByMACAddress {
    MKRMFilterByMacController *vc = [[MKRMFilterByMacController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByADVName {
    MKRMFilterByAdvNameController *vc = [[MKRMFilterByAdvNameController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByRawData {
    MKRMFilterByRawDataController *vc = [[MKRMFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)duplicateDataFilter {
    MKRMDuplicateDataFilterController *vc = [[MKRMDuplicateDataFilterController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataOption {
    MKRMUploadDataOptionController *vc = [[MKRMUploadDataOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSection1Datas {
    MKRMFilterCellModel *cellModel = [[MKRMFilterCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteMetering", @"MKRMUploadOptionController", @"rm_saveIcon.png")
...
- (MKRMUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMUploadOptionModel alloc] init];
    }
    return _dataModel;
}
