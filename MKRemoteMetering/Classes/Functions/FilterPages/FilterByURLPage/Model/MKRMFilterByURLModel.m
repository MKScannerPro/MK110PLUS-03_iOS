...
@interface MKRMFilterByURLModel ()
...
@implementation MKRMFilterByURLModel
...
- (BOOL)readFilterByURL {
    ...
    [MKRMMQTTInterface rm_readFilterByUrlWithMacAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByURL {
    __block BOOL success = NO;
    [MKRMMQTTInterface rm_configFilterByURL:self.isOn url:self.url macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
