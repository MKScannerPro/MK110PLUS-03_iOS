//
//  MKRMNormalConnectedController.m
...
@interface MKRMNormalConnectedController ()<UITableViewDelegate,
UITableViewDataSource,
MKRMNormalConnectedCellDelegate>
...
@property (nonatomic, strong)MKRMConnectedDeviceWriteAlertView *writeAlert;

@end

@implementation MKRMNormalConnectedController

- (void)dealloc {
    NSLog(@"MKRMNormalConnectedController销毁");
...
- (void)leftButtonMethod {
    //用户点击左上角，则需要返回MKRMDeviceDataController
    [self popToViewControllerWithClassName:@"MKRMDeviceDataController"];
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKRMNormalConnectedCellModel *cellModel = charList[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKRMNormalConnectedCell *cell = [MKRMNormalConnectedCell initCellWithTableView:tableView];
    NSDictionary *dic = self.sectionList[indexPath.section];
    NSArray *charList = dic[@"charList"];
...
#pragma mark - MKRMNormalConnectedCellDelegate
- (void)rm_normalConnectedCell_writeButtonPressed:(NSInteger)section
    ...
    @weakify(self);
    [self.writeAlert showAlertWithValue:@"" dismissNote:@"mk_rm_needDismissAlert" cancelAction:^{
...
- (void)receiveDeviceDatas:(NSNotification *)note {
    ...
    MKRMNormalConnectedCellModel *cellModel = charList[row];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDisconnect:)
                                                 name:MKRMReceiveGatewayDisconnectDeviceNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKRMReceiveGatewayConnectedDeviceDatasNotification
                                               object:nil];
}
...
- (void)parseTableDatas {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        for (NSInteger j = 0; j < charCount; j ++) {
            ...
            MKRMNormalConnectedCellModel *cellModel = [[MKRMNormalConnectedCellModel alloc] init];
            cellModel.section = i;
            cellModel.row = j;
...
- (void)disconnect {
    ...
    [MKRMMQTTInterface rm_disconnectNormalBleDeviceWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)writeData:(NSString *)data serviceUUID:(NSString *)serverUUID characteristic:(NSString *)characteristic {
    ...
    [MKRMMQTTInterface rm_writeValueToDeviceWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] value:data serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)readDataWithServiceUUID:(NSString *)serverUUID
    ...
    [MKRMMQTTInterface rm_readCharacteristicValueWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
characteristic:(NSString *)characteristic
    ...
    [MKRMMQTTInterface rm_notifyCharacteristic:notify bleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (MKRMConnectedDeviceWriteAlertView *)writeAlert {
    if (!_writeAlert) {
        _writeAlert = [[MKRMConnectedDeviceWriteAlertView alloc] init];
    }
    return _writeAlert;
}
