...
@interface MKRMBXPButtonController ()<UITableViewDelegate,
UITableViewDataSource,
MKButtonMsgCellDelegate,
MKRMButtonFirmwareCellDelegate>
...
@implementation MKRMBXPButtonController

- (void)dealloc {
    NSLog(@"MKRMBXPButtonController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKRMButtonFirmwareCell *cell = [MKRMButtonFirmwareCell initCellWithTableView:tableView];
...
#pragma mark - MKRMButtonFirmwareCellDelegate
- (void)rm_buttonFirmwareCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKRMButtonDFUController *vc = [[MKRMButtonDFUController alloc] init];
...
- (void)readDatasFromDevice {
    ...
    [MKRMMQTTInterface rm_readBXPButtonConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)dismissAlarmStatus {
    ...
    [MKRMMQTTInterface rm_dismissBXPButtonAlarmStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)loadSection1Datas {
    MKRMButtonFirmwareCellModel *cellModel = [[MKRMButtonFirmwareCellModel alloc] init];
    cellModel.index = 0;
