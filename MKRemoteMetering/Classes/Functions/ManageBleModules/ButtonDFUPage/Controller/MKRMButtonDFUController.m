...
@interface MKRMButtonDFUController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRMButtonDFUModel *dataModel;
...
@implementation MKRMButtonDFUController

- (void)dealloc {
    NSLog(@"MKRMButtonDFUController销毁");
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuProgress:)
                                                 name:MKRMReceiveBxpButtonDfuProgressNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuResult:)
                                                 name:MKRMReceiveBxpButtonDfuResultNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)gotoLastPage {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:NSClassFromString(@"MKRMManageBleDevicesController")]) {
            //如果是从Manage BLE devices页面进来的
            [self popToViewControllerWithClassName:@"MKRMManageBleDevicesController"];
            break;
        }
        if ([vc isKindOfClass:NSClassFromString(@"MKRMDeviceDataController")]) {
            //如果是从扫描页面进来的
...
- (MKRMButtonDFUModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRMButtonDFUModel alloc] init];
