//
//  MKRMManageBleDevicesController.m
...
@interface MKRMManageBleDevicesController ()<UITableViewDelegate,
UITableViewDataSource,
MKRMManageBleDevicesSearchButtonDelegate,
MKRMManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKRMManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKRMManageBleDevicesSearchButton *searchButton;
...
@implementation MKRMManageBleDevicesController

- (void)dealloc {
    NSLog(@"MKRMManageBleDevicesController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKRMManageBleDevicesCell *cell = [MKRMManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKRMManageBleDevicesSearchButtonDelegate
- (void)rm_scanSearchButtonMethod {
    [MKRMManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
- (void)rm_scanSearchButtonClearMethod {
...
#pragma mark - MKRMManageBleDevicesCellDelegate
/// 用户点击了链接按钮
...
- (void)rm_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode; {
    if (typeCode == 7) {
        ...
        [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_rm_needDismissAlert"];
        return;
    }
    //Normal
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKRMManageBleDevicesCellModel *cellModel = [[MKRMManageBleDevicesCellModel alloc] init];
...
- (void)connectBXPButtonWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    ...
    [MKRMMQTTInterface rm_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)connectNormalDeviceWithBleMac:(NSString *)macAddress {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKRMMQTTInterface rm_connectNormalBleDeviceWithBleMac:macAddress macAddress:[MKRMDeviceModeManager shared].macAddress topic:[MKRMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (MKRMManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKRMManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKRMManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKRMManageBleDevicesSearchButtonModel alloc] init];
