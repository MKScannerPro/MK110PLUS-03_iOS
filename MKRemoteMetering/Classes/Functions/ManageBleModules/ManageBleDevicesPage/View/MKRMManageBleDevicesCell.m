...
@implementation MKRMManageBleDevicesCellModel
@end

@interface MKRMManageBleDevicesCell ()
...
@implementation MKRMManageBleDevicesCell

+ (MKRMManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKRMManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRMManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKRMManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRMManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate rm_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRMManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMManageBleDevicesCellModel.class]) {
        return;
    }
