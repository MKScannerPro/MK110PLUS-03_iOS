//
//  MKRMDeviceDataPageHeaderView.m
...
@implementation MKRMDeviceDataPageHeaderViewModel
@end

@interface MKRMDeviceDataPageManageBleButton : UIControl
...
@implementation MKRMDeviceDataPageManageBleButton
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceDataPageManageBleButton", @"rm_goNextButton.png");
    }
    return _rightIcon;
}
...
@interface MKRMDeviceDataPageHeaderView ()
...
@property (nonatomic, strong)MKRMDeviceDataPageManageBleButton *manageBleButton;

@end

@implementation MKRMDeviceDataPageHeaderView
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_updateLoadButtonAction)]) {
        [self.delegate rm_updateLoadButtonAction];
    }
}

- (void)powerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_powerButtonAction)]) {
        [self.delegate rm_powerButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_scannerStatusChanged:)]) {
        [self.delegate rm_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)manageBleButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rm_manageBleDeviceAction)]) {
        [self.delegate rm_manageBleDeviceAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRMDeviceDataPageHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMDeviceDataPageHeaderViewModel.class]) {
        return;
    }
    self.scannerButton.selected = _dataModel.isOn;
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"MKRemoteMetering", @"MKRMDeviceDataPageHeaderView", @"rm_switchSelectedIcon.png") : LOADICON(@"MKRemoteMetering", @"MKRMDeviceDataPageHeaderView", @"rm_switchUnselectedIcon.png"));
...
- (MKRMDeviceDataPageManageBleButton *)manageBleButton {
    if (!_manageBleButton) {
        _manageBleButton = [[MKRMDeviceDataPageManageBleButton alloc] init];
