...
static NSString *const localPasswordKey = @"mk_rm_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKRMScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_rm_centralManagerScanDelegate>
...
@implementation MKRMScanPageController

- (void)dealloc {
    NSLog(@"MKRMScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKRMCentralManager shared] stopScan];
    [MKRMCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKRMCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKRMCentralManager shared].centralStatus != mk_rm_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKRMScanPageCell *cell = [MKRMScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_rm_centralManagerScanDelegate

- (void)mk_rm_receiveDevice:(NSDictionary *)deviceModel {
    MKRMScanPageModel *dataModel = [MKRMScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKRMCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKRMScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKRMScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKRMCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKRMScanPageModel *)deviecModel {
    ...
    [[MKRMCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (void)pushMQTTForDevicePage:(NSString *)deviceType {
    MKRMDeviceParamsListController *vc = [[MKRMDeviceParamsListController alloc] init];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMScanPageController", @"rm_scanRefresh.png");
    }
    return _refreshIcon;
}
