...
@implementation MKRMDeviceListCell

+ (MKRMDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKRMDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRMDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKRMDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRMDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(rm_cellDeleteButtonPressed:)]) {
        [self.delegate rm_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRMDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKRMDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKRMDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKRMDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceListCell", @"rm_offline_wifi.png");
        return;
    }
    //设备在线
    if (_dataModel.wifiLevel >= -50) {
        //Good
        self.wifiIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceListCell", @"rm_good_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel >= -65 && _dataModel.wifiLevel < -50) {
        //Medium
        self.wifiIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceListCell", @"rm_medium_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel < -65) {
        //Poor
        self.wifiIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceListCell", @"rm_poor_wifi.png");
        return;
    }
}
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceListCell", @"rm_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKRemoteMetering", @"MKRMDeviceListCell", @"rm_goNextButton.png");
    }
    return _rightIcon;
}
