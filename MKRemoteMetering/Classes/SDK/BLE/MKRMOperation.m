...
/**
 线程ID
 */
@property (nonatomic, assign)mk_rm_taskOperationID operationID;
...
- (instancetype)initOperationWithID:(mk_rm_taskOperationID)operationID
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKRMTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKRMTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_rm_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_rm_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    //分帧数据
    NSString *totalNum = returnData[mk_rm_totalNumKey];
    if (MKValidStr(totalNum)) {
        //存在分帧数据
        self.receiveTimerCount = 0;
        NSData *data = returnData[mk_rm_contentKey];
        if (data) {
