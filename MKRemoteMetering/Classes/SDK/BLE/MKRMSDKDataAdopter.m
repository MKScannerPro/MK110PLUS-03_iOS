...
@implementation MKRMSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_rm_wifiSecurity)security {
    switch (security) {
        case mk_rm_wifiSecurity_personal:
            return @"00";
        case mk_rm_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_rm_eapType)eapType {
    switch (eapType) {
        case mk_rm_eapType_peap_mschapv2:
            return @"00";
        case mk_rm_eapType_ttls_mschapv2:
            return @"01";
        case mk_rm_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_rm_connectMode)mode {
    switch (mode) {
        case mk_rm_connectMode_TCP:
            return @"00";
        case mk_rm_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_rm_connectMode_CACertificate:
            return @"02";
        case mk_rm_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_rm_mqttServerQosMode)mode {
    switch (mode) {
        case mk_rm_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_rm_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_rm_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
