...
#import "ZipArchive.h"
#import "MKRMXMLReader.h"

@interface MKRMExcelWookbook()
...
@implementation MKRMExcelWookbook
                ...
                if(writeSuccess)
                {
                    ...
                    if(uncompressionSuccess)
                    {
                        ...
                        for(NSInteger i = 0 ; i < array.count ; i ++)
                        {
                            ...
                            MKRMExcelSheet *sheet = [[MKRMExcelSheet alloc]init];
                            
                            sheet.sheetId = [[sheetDic objectForKey:@"sheetId"] integerValue];
                            
                            sheet.cellArray = [MKRMExcelSheet analysisSheetDataWithSheetDic:sheetDic sharedStringsArray:stringValueArray];
...
-(NSDictionary *)getSharedStringsWithPath:(NSString *)path
{
    ...
    if(xmlData && xmlData.length > 0)
    {
        ...
        xmlDic = [MKRMXMLReader dictionaryForXMLData:xmlData error:&error];
    }
    return xmlDic;
}
    ...
    while (fileName = [dirEnum nextObject]) {
        ...
        if ([fileName hasSuffix:@".xml"]) {
            ...
            if(xmlData && xmlData.length > 0)
            {
                ...
                NSInteger sheetId = [[MKRMExcelCell getNumberFromStr:fileName] integerValue];
                
                NSMutableDictionary *sheetDic = [[NSMutableDictionary alloc]init];
                
                NSDictionary *xmlDic = [MKRMXMLReader dictionaryForXMLData:xmlData error:&error];
...
-(NSArray *)getSheedNameInfoArrayWithPath:(NSString *)path
{
    ...
    NSDictionary *sheetNameInfoDic = [MKRMXMLReader dictionaryForXMLData:xmlData error:&error];
