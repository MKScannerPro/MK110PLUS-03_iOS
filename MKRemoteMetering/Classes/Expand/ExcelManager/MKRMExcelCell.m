...
-(void)setMergeCellColumAndRowStr:(NSString *)mergeCellColumAndRowStr
{
    ...
    if(mergeCellColumAndRowStr && [mergeCellColumAndRowStr isKindOfClass:[NSString class]] &&mergeCellColumAndRowStr.length > 0)
    {
        ...
        NSString *rowStr = [MKRMExcelCell getNumberFromStr:mergeCellColumAndRowStr];
        
        self.mergeRow = [rowStr integerValue];
        
        NSString *column = [MKRMExcelCell getLetterFromStr:mergeCellColumAndRowStr];
...
-(void)refreshData
{
    ...
    NSString *rowStr = [MKRMExcelCell getNumberFromStr:r];
    
    self.row = [rowStr integerValue];
    
    NSString *column = [MKRMExcelCell getLetterFromStr:r];
