...
#import "MKRMExcelWookbook.h"

static NSString *const defaultKeyValueString = @"value:";

@implementation MKRMExcelDataManager

+ (void)exportAppExcel:(id <MKRMExcelAppProtocol>)protocol
              sucBlock:(void(^)(void))sucBlock
           failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRMExcelAppProtocol)]) {
...
+ (void)parseAppExcel:(NSString *)excelName
    ...
    MKRMExcelWookbook *workbook = [[MKRMExcelWookbook alloc] initWithExcelFilePathUrl:excelUrl];
    if (!workbook || workbook.sheetArray.count == 0) {
        [self operationFailedBlockWithMsg:@"Load Excel Data Failed" block:failedBlock];
        return;
    }
    MKRMExcelSheet *sheet = workbook.sheetArray.firstObject;
    if (![sheet isKindOfClass:MKRMExcelSheet.class]) {
        [self operationFailedBlockWithMsg:@"Load Excel Data Failed" block:failedBlock];
        return;
    }
    ...
    //根据横竖坐标，获取单元格
    //Host
    MKRMExcelCell *hostCell = [sheet getCellWithColumn:@"B" row:2 error:nil];
    ...
    //Port
    MKRMExcelCell *portCell = [sheet getCellWithColumn:@"B" row:3 error:nil];
    ...
    //Client id
    MKRMExcelCell *clientIDCell = [sheet getCellWithColumn:@"B" row:4 error:nil];
    ...
    //Subscribe Topic
    MKRMExcelCell *subTopicCell = [sheet getCellWithColumn:@"B" row:5 error:nil];
    ...
    //Publish Topic
    MKRMExcelCell *pubTopicCell = [sheet getCellWithColumn:@"B" row:6 error:nil];
    ...
    //Clean Session
    MKRMExcelCell *cleanSessionCell = [sheet getCellWithColumn:@"B" row:7 error:nil];
    ...
    //Qos
    MKRMExcelCell *qosCell = [sheet getCellWithColumn:@"B" row:8 error:nil];
    ...
    //Keep Alive
    MKRMExcelCell *keepAliveCell = [sheet getCellWithColumn:@"B" row:9 error:nil];
...
//    }
    ...
    //MQTT Username
    MKRMExcelCell *usernameCell = [sheet getCellWithColumn:@"B" row:10 error:nil];
    ...
    //MQTT Password
    MKRMExcelCell *passwordCell = [sheet getCellWithColumn:@"B" row:11 error:nil];
    ...
    //SSL/TLS
    MKRMExcelCell *sslCell = [sheet getCellWithColumn:@"B" row:12 error:nil];
    ...
    //Certificate type
    MKRMExcelCell *certificateCell = [sheet getCellWithColumn:@"B" row:13 error:nil];
...
+ (void)exportDeviceExcel:(id <MKRMExcelDeviceProtocol>)protocol
                 sucBlock:(void(^)(void))sucBlock
              failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRMExcelDeviceProtocol)]) {
        [self operationFailedBlockWithMsg:@"Params Error" block:failedBlock];
        return;
    }
...
//    }
    ...
    [dic setObject:@(certValue) forKey:@"certificate"];
    //LWT
    MKRMExcelCell *lwtCell = [sheet getCellWithColumn:@"B" row:14 error:nil];
    ...
    //LWT Retain
    MKRMExcelCell *lwtRetainCell = [sheet getCellWithColumn:@"B" row:15 error:nil];
    ...
    //LWT Qos
    MKRMExcelCell *lwtQosCell = [sheet getCellWithColumn:@"B" row:16 error:nil];
    ...
    //LWT Topic
    MKRMExcelCell *lwtTopicCell = [sheet getCellWithColumn:@"B" row:17 error:nil];
    ...
    //LWT Payload
    MKRMExcelCell *lwtPayloadCell = [sheet getCellWithColumn:@"B" row:18 error:nil];
...
+ (BOOL)checkExcelAppProtocol:(id <MKRMExcelAppProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRMExcelAppProtocol)]) {
        return NO;
    }
...
+ (BOOL)checkExcelDeviceProtocol:(id <MKRMExcelDeviceProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRMExcelDeviceProtocol)]) {
        return NO;
    }
    if (!ValidStr(protocol.host) || protocol.host.length > 64 || ![protocol.host isAsciiString]) {
