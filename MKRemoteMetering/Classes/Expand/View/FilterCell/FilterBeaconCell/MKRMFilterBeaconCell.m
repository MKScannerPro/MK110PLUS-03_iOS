...
@implementation MKRMFilterBeaconCellModel
@end

@interface MKRMFilterBeaconCell ()
...
@implementation MKRMFilterBeaconCell

+ (MKRMFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKRMFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRMFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKRMFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRMFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKRMFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_rm_beaconMinValueChanged:index:)]) {
                [self.delegate mk_rm_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_rm_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_rm_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
