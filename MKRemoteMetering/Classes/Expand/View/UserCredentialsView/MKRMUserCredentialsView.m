...
@implementation MKRMUserCredentialsViewModel
@end

@interface MKRMUserCredentialsView ()
...
@implementation MKRMUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKRMUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRMUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rm_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate rm_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rm_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate rm_mqtt_userCredentials_passwordChanged:text];
            }
        };
