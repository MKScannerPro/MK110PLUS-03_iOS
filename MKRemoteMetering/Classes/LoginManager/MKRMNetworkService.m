...
@implementation MKRMCreatScannerProDeviceModel
...
@interface MKRMNetworkService ()

@property (nonatomic, strong)NSURLSessionDataTask *addScannerProDeviceTask;

@end

@implementation MKRMNetworkService

+ (instancetype)share {
    static dispatch_once_t t;
    static MKRMNetworkService *service = nil;
    dispatch_once(&t, ^{
        service = [[MKRMNetworkService alloc] init];
    });
    return service;
}

- (void)addScannerProDevicesToCloud:(NSArray <MKRMCreatScannerProDeviceModel *>*)deviceList
                             isHome:(BOOL)isHome
    ...
    for (MKRMCreatScannerProDeviceModel *deviceModel in deviceList) {
